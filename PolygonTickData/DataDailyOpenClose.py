...
class DailyOpenCloseData(object):
	...
	def createUrls(self, symbolsToBeDownloaded=None):
		...
		return [createUrls.PolygonAggregdateData(symbol=symbol,aggregateBy='day',startDate=self.date, endDate=endDate) for symbol in symbolsToBeDownloaded]
	...
	def getSaveOpenCloseData(self,openCloseURLs=None):
		...
		for response in responses:
			...
			try:
				self.dailyopencloseObj.insertIntoCollection(symbol=symbol, datetosave=self.date, savedata=responseData[0], 
