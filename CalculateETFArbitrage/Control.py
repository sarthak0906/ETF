...
class ArbitrageCalculation():
    def calculateArbitrage(self, etfname, date):
        ...
        helperObj = Helper()
        allData.tradesDataDf['Time'] = allData.tradesDataDf['Time'].apply(
            lambda x: helperObj.getHumanTime(ts=x, divideby=1000))
        tradePricesDFMinutes = \
        allData.tradesDataDf.groupby([allData.tradesDataDf['Time'], allData.tradesDataDf['Symbol']])['Trade Price']
...
if __name__=="__main__":
    ds = ArbitrageCalculation().calculateArbitrage('XLK', '2020-03-16')
    print(ds.head())
    print(ds.columns)
